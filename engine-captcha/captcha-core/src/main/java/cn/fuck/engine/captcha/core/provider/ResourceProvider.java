package cn.fuck.engine.captcha.core.provider;

import cn.fuck.engine.assistant.core.utils.ResourceUtils;
import cn.fuck.engine.captcha.core.definition.enums.CaptchaResource;
import cn.fuck.engine.captcha.core.definition.enums.FontStyle;
...
public class ResourceProvider implements InitializingBean {
    ...
    private static final String FONT_FOLDER = "/usr/share/fonts/fuck/";
    ...
    private static Map<String, String> getImages(String location) {
        if (ResourceUtils.isClasspathAllUrl(location)) {
            try {
                ...
                log.error("[FUCK] |- Analysis the  location [{}] catch io error!", location, e);
            }
        }
    ...
    private static Font getFont(Resource resource) {
        ...
        try {
            ...
            log.warn("[FUCK] |- Can not read font in the resources folder, maybe in docker.");
            ...
            log.error("[FUCK] |- Resource object in resources folder catch io error!", e);
        }
        ...
        if (ManagementUtil.getOsInfo().isLinux()) {
            ...
            if (ObjectUtils.isNotEmpty(file) && FileUtil.exists(file)) {
                ...
                try {
                    ...
                    log.debug("[FUCK] |- Read font [{}] under the DOCKER.", font.getFontName());
                    return font;
                } catch (IORuntimeException e) {
                    log.error("[FUCK] |- Read font under the DOCKER catch error.");
                } catch (NullPointerException e) {
                    log.error("[FUCK] |- Read font under the DOCKER catch null error.");
                }
            }
        }
        return null;
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        log.debug("[FUCK] |- Before captcha resource loading, check system. Current system is [{}]", systemName);

        log.debug("[FUCK] |- Captcha resource loading is BEGIN！");
        ...
        log.debug("[FUCK] |- Jigsaw captcha resource loading is END！");
    }
    ...
    private void loadImages(Map<String, String> container, String location, CaptchaResource captchaResource) {
        ...
        if (MapUtils.isNotEmpty(resource)) {
            ...
            log.debug("[FUCK] |- {} load complete, total number is [{}]", captchaResource.getContent(), resource.size());
    ...
    private void loadFonts() {
        if (MapUtils.isEmpty(fonts)) {
            ...
            log.debug("[FUCK] |- Font load complete, total number is [{}]", fonts.size());
        }
    }
