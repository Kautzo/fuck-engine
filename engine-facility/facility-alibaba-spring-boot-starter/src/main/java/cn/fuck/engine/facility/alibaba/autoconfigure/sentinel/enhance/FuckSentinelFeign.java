                ...
                public InvocationHandler create(Target target,
                    ...
                    if (void.class != fallback) {
                        fallbackInstance = getFromContext(beanName, "fallback", fallback,
                                target.type());
                        return new FuckSentinelInvocationHandler(target, dispatch, new FallbackFactory.Default(fallbackInstance));
                    }
                    if (void.class != fallbackFactory) {
                        fallbackFactoryInstance = (FallbackFactory) getFromContext(
                                beanName, "fallbackFactory", fallbackFactory,
                                FallbackFactory.class);
                        return new FuckSentinelInvocationHandler(target, dispatch, fallbackFactoryInstance);
                    }

                    // 默认的 fallbackFactory
                    FuckFallbackFactory fuckFallbackFactory = new FuckFallbackFactory(target);
                    return new FuckSentinelInvocationHandler(target, dispatch, fuckFallbackFactory);
                }
