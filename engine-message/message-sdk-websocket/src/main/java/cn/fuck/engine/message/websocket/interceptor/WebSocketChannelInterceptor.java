...
public class WebSocketChannelInterceptor implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        if (ObjectUtils.isEmpty(principal)) {
            log.warn("[FUCK] |- WebSocket channel cannot fetch user principal.");
            return null;
        }
        ...
        if (ObjectUtils.isNotEmpty(command)) {
            switch (command) {
                ...
                case CONNECT -> {
                    ...
                    log.debug("[FUCK] |- Authentication user [{}] transmit token [{}] from frontend.", user.getName(), token);
                }
                case DISCONNECT -> {
                }
                default -> {
