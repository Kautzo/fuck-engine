package cn.fuck.engine.assistant.core.utils;
...
import cn.fuck.engine.assistant.core.exception.properties.PropertyValueIsNotSetException;
...
public class WellFormedUtils {
    ...
    public static String getHostAddress() {
        ...
        try {
            ...
            log.error("[FUCK] |- Get host address error: {}", e.getLocalizedMessage());
            return null;
        }
    }
    ...
    public static String serviceUri(String serviceUri, String serviceName, String gatewayServiceUri, String abbreviation) {
        if (StringUtils.isNotBlank(serviceUri)) {
            ...
            if (StringUtils.isBlank(serviceName)) {
                log.error("[FUCK] |- Property [{} Service Name] is not set or property format is incorrect!", abbreviation);
                throw new PropertyValueIsNotSetException();
            } else {
                if (StringUtils.isBlank(gatewayServiceUri)) {
                    log.error("[FUCK] |- Property [gateway-service-uri] is not set or property format is incorrect!");
                    throw new PropertyValueIsNotSetException();
                } else {
    ...
    public static String sasUri(String uri, String endpoint, String issuerUri) {
        if (StringUtils.isNotBlank(uri)) {
            ...
            if (StringUtils.isBlank(issuerUri)) {
                log.error("[FUCK] |- Property [issuer-uri] is not set or property format is incorrect!");
                throw new PropertyValueIsNotSetException();
            } else {
