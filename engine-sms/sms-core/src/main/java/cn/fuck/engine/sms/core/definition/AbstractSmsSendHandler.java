...
public abstract class AbstractSmsSendHandler implements SmsSendHandler {
    ...
    public boolean send(Template template, List<String> phones) {
        ...
        try {
            ...
            if (isSuccessful) {
                log.debug("[FUCK] |- [{}] ：Send sms is successfully!", this.getChannel());
            } else {
                log.error("[FUCK] |- [{}] ：Send sms is failed!", this.getChannel());
            }
            return isSuccessful;
        } catch (TemplateIdInvalidException e) {
            log.error("[FUCK] |- [{}] : TemplateId [{}] is invalid, Please ensure correct configuration!", this.getChannel(), type);
        } catch (ParameterOrdersInvalidException e) {
            log.error("[FUCK] |- [{}] : Parameter order for template [{}] is invalid, Please ensure correct configuration!", this.getChannel(), type);
        }
