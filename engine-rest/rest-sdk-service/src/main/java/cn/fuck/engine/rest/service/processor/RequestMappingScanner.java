package cn.fuck.engine.rest.service.processor;
...
public class RequestMappingScanner implements ApplicationListener<ApplicationReadyEvent> {
    ...
    public void onApplicationEvent(ApplicationReadyEvent event) {
        ...
        log.debug("[FUCK] |- [1] Application is READY, start to scan request mapping!");
    ...
    public void onApplicationEvent(ApplicationContext applicationContext) {
        ...
        if (!requestMappingScanStrategyEvent.isPerformScan()) {
            // 只扫描资源服务器
            log.warn("[FUCK] |- Can not found scan annotation in Service [{}], Skip!", serviceId);
            return;
        }
        ...
        if (CollectionUtils.isNotEmpty(resources)) {
            log.debug("[FUCK] |- [2] Request mapping scan found [{}] resources in service [{}], go to next stage!", serviceId, resources.size());
            requestMappingScanStrategyEvent.postProcess(resources);
        } else {
            log.debug("[FUCK] |- [2] Request mapping scan can not find any resources in service [{}]!", serviceId);
        }

        log.info("[FUCK] |- Request Mapping Scan for Service: [{}] FINISHED!", serviceId);
    }
    ...
    private String createCode(String url, String requestMethods) {
        ...
        log.trace("[FUCK] |- Create code [{}] for Request [{}] : [{}]", result, requestMethods, url);
        return result;
    }
