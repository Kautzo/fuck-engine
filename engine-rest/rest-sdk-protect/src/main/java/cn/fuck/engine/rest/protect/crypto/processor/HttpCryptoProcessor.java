...
import cn.fuck.engine.cache.core.exception.StampHasExpiredException;
...
public class HttpCryptoProcessor extends AbstractStampManager<String, SecretKey> {
    ...
    public String encrypt(String identity, String content) throws SessionInvalidException {
        try {
            ...
            log.debug("[FUCK] |- Encrypt content from [{}] to [{}].", content, result);
            return result;
        } catch (StampHasExpiredException e) {
            log.warn("[FUCK] |- Session has expired, need recreate.");
            throw new SessionInvalidException();
        } catch (Exception e) {
            log.warn("[FUCK] |- Symmetric can not Encrypt content [{}], Skip!", content);
            return content;
        }
    }
    ...
    public String decrypt(String identity, String content) throws SessionInvalidException {
        try {
            ...
            log.debug("[FUCK] |- Decrypt content from [{}] to [{}].", content, result);
            return result;
        } catch (StampHasExpiredException e) {
            log.warn("[FUCK] |- Session has expired, need recreate.");
            throw new SessionInvalidException();
        } catch (Exception e) {
            log.warn("[FUCK] |- Symmetric can not Decrypt content [{}], Skip!", content);
            return content;
        }
    }
    ...
    public SecretKey nextStamp(String key) {
        ...
        log.debug("[FUCK] |- Generate secret key, value is : [{}]", secretKey);
        return secretKey;
    }
    ...
    private SecretKey getSecretKey(String identity) throws StampHasExpiredException {
        if (isSessionValid(identity)) {
            ...
            if (ObjectUtils.isNotEmpty(secretKey)) {
                log.trace("[FUCK] |- Decrypt Or Encrypt content use param identity [{}], cached identity is [{}].", identity, secretKey.getIdentity());
                return secretKey;
            }
        }
    ...
    private String decryptFrontendPublicKey(String content, String privateKey) {
        ...
        log.debug("[FUCK] |- Decrypt frontend public key, value is : [{}]", frontendPublicKey);
    ...
    private String encryptBackendKey(String symmetricKey, String publicKey) {
        ...
        log.debug("[FUCK] |- Encrypt symmetric key use frontend public key, value is : [{}]", encryptedAesKey);
