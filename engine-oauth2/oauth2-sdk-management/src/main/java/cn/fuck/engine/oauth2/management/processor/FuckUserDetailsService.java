...
import cn.fuck.engine.oauth2.core.definition.strategy.StrategyUserDetailsService;
 ...
 */
public class FuckUserDetailsService implements EnhanceUserDetailsService {

    private static final Logger log = LoggerFactory.getLogger(FuckUserDetailsService.class);

    private final StrategyUserDetailsService strategyUserDetailsService;

    public FuckUserDetailsService(StrategyUserDetailsService strategyUserDetailsService) {
...
//    }
    ...
    public UserDetails loadUserBySocial(String source, AccessPrincipal accessPrincipal) throws UsernameNotFoundException {
        FuckUser FuckUser = strategyUserDetailsService.findUserDetailsBySocial(StringUtils.toRootUpperCase(source), accessPrincipal);
        log.debug("[FUCK] |- UserDetailsService loaded social user : [{}]", FuckUser.getUsername());
        return FuckUser;
    }

    @Override
    public FuckUser loadFuckUserByUsername(String username) throws UsernameNotFoundException {
        FuckUser FuckUser = strategyUserDetailsService.findUserDetailsByUsername(username);
        log.debug("[FUCK] |- UserDetailsService loaded user : [{}]", username);
        return FuckUser;
    }
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        return loadFuckUserByUsername(username);
    }
}
