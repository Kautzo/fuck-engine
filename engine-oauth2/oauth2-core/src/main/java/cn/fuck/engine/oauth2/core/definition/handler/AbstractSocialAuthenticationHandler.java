...
import cn.fuck.engine.oauth2.core.exception.SocialCredentialsUserBindingFailedException;
import cn.fuck.engine.oauth2.core.exception.UsernameAlreadyExistsException;
...
public abstract class AbstractSocialAuthenticationHandler implements SocialAuthenticationHandler {
     ...
     * @return 系统用户 {@link FuckUser}
     * @throws UsernameAlreadyExistsException 用户名已经存在
     */
    public abstract FuckUser register(SocialUserDetails socialUserDetails) throws UsernameAlreadyExistsException;
     ...
     *
     * @param FuckUser     系统用户信息 {@link FuckUser}
     * @param socialUserDetails 社交登录过程中，第三方系统返回的新信息
     */
    public abstract void additionalRegisterOperation(FuckUser FuckUser, SocialUserDetails socialUserDetails);
     ...
     * @return 系统用户 {@link FuckUser}
     */
    public abstract FuckUser signIn(SocialUserDetails socialUserDetails);
     ...
     */
    public abstract void additionalSignInOperation(FuckUser FuckUser, SocialUserDetails newSocialUserDetails, SocialUserDetails oldSocialUserDetails);
     ...
     */
    @Override
    public FuckUser authentication(String source, AccessPrincipal accessPrincipal) throws AuthenticationException {
        ...
        if (ObjectUtils.isEmpty(oldSocialUserDetails)) {
            FuckUser FuckUser = this.register(newSocialUserDetails);
            this.binding(FuckUser.getUserId(), newSocialUserDetails);
            this.additionalRegisterOperation(FuckUser, newSocialUserDetails);
            return FuckUser;
        } else {
            FuckUser FuckUser = this.signIn(oldSocialUserDetails);
            this.additionalSignInOperation(FuckUser, newSocialUserDetails, oldSocialUserDetails);
            return FuckUser;
        }
    }
}
